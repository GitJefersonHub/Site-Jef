...
function formatarTempo(tempo) {
    ...
    return ` ${dias} dias, ${horas}: ${minutos}: ${segundos}`; // Retorna o tempo formatado
}
...
function diadosorteio(dia, mes, ano, hora) {
    ...
    const intervalId = setInterval(() => {
        ...
        if (diferenca > 0) {
            ...
            document.getElementById("cronometro").textContent = "Parabéns"; // Exibe mensagem ao finalizar
...
if (!sorteioRealizado) {
    diadosorteio(28, 5, 2025, 21); // Configura a data e hora do sorteio
} else {
    document.getElementById("cronometro").textContent = "Serviço social:"; // Mensagem caso o sorteio já tenha sido realizado
